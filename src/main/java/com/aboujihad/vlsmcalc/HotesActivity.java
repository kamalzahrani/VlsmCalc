...
public class HotesActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_hotes);
        ...
        et_nbrOfMachines = findViewById(R.id.input_nbrOfMachines);
        ...
        et_mask4.setOnKeyListener(new View.OnKeyListener() {
            ...
            public boolean onKey(View v, int keyCode, KeyEvent event) {
                if ((event.getAction() == KeyEvent.ACTION_UP) &&
                    ...
                    //do code
                    et_nbrOfMachines.requestFocusFromTouch();
                    return true;
    ...
    public void showResultForHotesSegmentation(View view) {
        ...
        String str_nbrOfMachines = et_nbrOfMachines.getText().toString();
        ...
        int nbrOfMachines = Integer.parseInt(str_nbrOfMachines);

        int[] ip_saisi = {oct1, oct2, oct3, oct4};
        int[] msk_saisi = {mask1, mask2, mask3, mask4};


        if(MyException.checkIp(ip_saisi).matches("") & MyException.checkMask(msk_saisi).matches("") & MyException.chekSubNetScope(ip_saisi, msk_saisi, nbrOfMachines).matches("")){
            ...
            ArrayList<int[]> allSubnet=ip.getAllSubnet(ip_saisi, msk_saisi, ip.getNbrOfSubnet(msk_saisi, nbrOfMachines));

            for(int i=0;i<allSubnet.size();i++){
                listOfVlsmInfo.add("Subnet Id : "+(i+1)+"\n" +
                        "Subnet Address : "+ip.getIptoString(allSubnet.get(i))+"\n" +
                        "Subnet Mask : "+ip.getIptoString(ip.getNewMask(ip_saisi, msk_saisi, ip.getNbrOfSubnet(msk_saisi, nbrOfMachines)))+"\n" +
                        "Number Of Use Address : "+ip.getNbrOfUseAddr(ip.getNewMask(ip_saisi, msk_saisi, ip.getNbrOfSubnet(msk_saisi, nbrOfMachines)))+"\n" +
                        "First Address : "+ip.getIptoString(ip.getFirstAddress(allSubnet.get(i), ip.getNewMask(ip_saisi, msk_saisi, ip.getNbrOfSubnet(msk_saisi, nbrOfMachines))))+"\n" +
                        "Last Address : "+ip.getIptoString(ip.getLastAddress(allSubnet.get(i), ip.getNewMask(ip_saisi, msk_saisi, ip.getNbrOfSubnet(msk_saisi, nbrOfMachines))))+"\n" +
                        "Broadcast Address : "+ip.getIptoString(ip.getBroadCastAddress(allSubnet.get(i), ip.getNewMask(ip_saisi, msk_saisi, ip.getNbrOfSubnet(msk_saisi, nbrOfMachines))))+"\n" +
                        "\n");

            }

            // starting new activity
            Intent resultActivity = new Intent(HotesActivity.this, ResultActivity.class);
